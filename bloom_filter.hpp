...
static HashFunction h1(13);
static HashFunction h2(17);
static HashFunction h3(31);
static HashFunction h4(41);
static HashFunction h5(53);
static HashFunction defaultHashFunctions[5] = {h1, h2, h3, h4, h5};
...
class BloomFilter {
 public:
    BloomFilter(unsigned int bitsPerElement = 10,
        ...
        HashFunction hashFunctions[] = defaultHashFunctions,
        int numHashFunctions = sizeof(defaultHashFunctions)/sizeof(defaultHashFunctions[0]));
 ...
 private:
   ...
   * @param lastHash Input and output for the last hash value
  ...
  void getHashesForCharCodes(const char *input, int inputLen,
      uint64_t *lastHash, uint64_t *newHashes, unsigned char lastCharCode);
};
